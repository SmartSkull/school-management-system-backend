...
async function run() {
  ...
  try {
    ...
    for (const row of staffRows) {
      ...
      staffUserInserts.push([uniqueId, 'STAFF', firstName, lastName, email, telephone, password, image, 'ACTIVE']);
      staffInserts.push([uniqueId, stateOfOrigin, dateOfBirth || null, homeAddress, about]);
    }
    const [staffUserRes] = await conn.query(
      'INSERT INTO User (uniqueId, role, firstName, lastName, email, telephone, password, image, status, createdAt, updatedAt) VALUES ?',
      [staffUserInserts]
    );
    const newStaffIds = Array.from({length: staffUserInserts.length}, (_, i) => staffUserRes[0].insertId + i);
    for (let i = 0; i < staffInserts.length; i++) {
      staffInserts[i][0] = newStaffIds[i];
      userMap[staffInserts[i][0]] = newStaffIds[i];
      // Actually userMap should map uniqueId -> userId
      userMap[staffUserInserts[i][0]] = newStaffIds[i];
    }
    const [staffRes] = await conn.query(
      'INSERT INTO Staff (userId, staffNo, stateOfOrigin, dateOfBirth, homeAddress, about, createdAt, updatedAt) VALUES ?',
      [staffInserts]
    );
    console.log('Staff:', staffUserInserts.length, 'users + staff records');
    ...
    for (const row of usersRows) {
      ...
      if (className) classNameSet.add(className);
      userInserts.push([studentId, 'STUDENT', firstName, lastName, email, telephone, password, image, 'ACTIVE']);
      studentInserts.push([studentId, className, admissionYear, dateOfBirth || null, stateOfOrigin, homeAddress, fatherName, motherName, parentImage, about]);
    }
    const [stuRes] = await conn.query(
      'INSERT INTO User (uniqueId, role, firstName, lastName, email, telephone, password, image, status, createdAt, updatedAt) VALUES ?',
      [userInserts]
    );
    const newStuIds = Array.from({length: userInserts.length}, (_, i) => stuRes[0].insertId + i);
    for (let i = 0; i < newStuIds.length; i++) {
      userMap[userInserts[i][0]] = newStuIds[i];
    }
    
    // Map class names to IDs for classes that exist in target but not in source class table
    const [allClasses] = await conn.execute('SELECT id, name FROM ClassRoom WHERE schoolId = ?', [SCHOOL_ID]);
    for (const c of allClasses) {
      if (!classMap[c.name]) classMap[c.name] = c.id;
      if (classNameSet.has(c.name)) classNameSet.delete(c.name);
    }
    // Also check global classes
    ...
    const finalStudentInserts = studentInserts.map((s, i) => {
      const className = s[1];
      let classRoomId = null;
      if (className && classMap[className]) classRoomId = classMap[className];
      return [newStuIds[i], userInserts[i][0], classRoomId, s[2], s[3], s[4], s[5], s[6], s[7], s[8], s[9]];
    });
    
    await conn.query(
      'INSERT INTO Student (userId, studentNo, classRoomId, admissionYear, dateOfBirth, stateOfOrigin, homeAddress, fatherName, motherName, parentImage, about, createdAt, updatedAt) VALUES ?',
      [finalStudentInserts]
    );
    console.log('Students:', userInserts.length);
