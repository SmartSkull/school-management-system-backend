...
export class AdminService {
  ...
  async dashboard() {
    const [totalStudents, verifiedStudents, pendingStudents, totalStaff, verifiedStaff, session, term, recentStudents, recentPayments, grouped] = await Promise.all([
      this.prisma.count('users'),
      this.prisma.count('users', { admin_verify: '1' }),
      this.prisma.count('users', { admin_verify: '0' }),
      this.prisma.count('staff'),
      this.prisma.count('staff', { admin_verify: '1' }),
      this.prisma.findFirst<any>('set_session_tbl'),
      this.prisma.findFirst<any>('set_term_tbl'),
      this.prisma.findMany('users', { orderBy: { user_id: 'desc' }, take: 3, select: { firstname: true, lastname: true, date: true } }),
      this.prisma.findMany('scratch_card', { orderBy: { scratch_card_id: 'desc' }, take: 3, select: { student_id: true, date: true } }),
      this.prisma.groupBy<any>('users', { by: ['class'], _count: { _all: true } }),
    ]);
  ...
  async getStudents(q: any) {
    ...
    const [total, students] = await Promise.all([
      this.prisma.count('users', where),
      this.prisma.findMany('users', {
        where,
  ...
  async updateStudent(studentId: string, data: any) {
    const student = await this.prisma.findFirst('users', { where: { student_id: studentId } });
    ...
    if (Object.keys(update).length) await this.prisma.updateMany('users', { student_id: studentId }, update);
    return this.ok(null, 'Student updated successfully');
  }

  async verifyStudent(studentId: string) {
    await this.prisma.updateMany('users', { student_id: studentId }, { admin_verify: '1' });
    await this.prisma.create('notifications', { user_id: studentId, user_type: 'student', title: 'Account Verified', message: 'Your account has been verified. You can now access all features.', is_read: false, created_at: new Date() });
  ...
  async deleteStudent(studentId: string) {
    const student = await this.prisma.findFirst<any>('users', { where: { student_id: studentId }, select: { user_id: true } });
    if (!student) throw new NotFoundException('Student not found');
    await this.prisma.deleteMany('users', { user_id: student.user_id });
  ...
  async getStaff(q: any) {
    ...
    const [total, staff] = await Promise.all([
      this.prisma.count('staff'),
      this.prisma.findMany('staff', { orderBy: { staff_id: 'desc' }, take: perPage, skip: (page - 1) * perPage }),
    ]);
  ...
  async createStaff(data: any) {
    ...
    await this.prisma.create('staff', {
      unique_id: uniqueId,
      firstname: data.firstname || '',
      lastname: data.lastname || '',
      email: data.email || '',
  ...
  async updateStaff(staffId: string, data: any) {
    ...
    if (Object.keys(update).length) await this.prisma.updateMany('staff', { unique_id: staffId }, update);
    return this.ok(null, 'Staff updated successfully');
  }

  async verifyStaff(staffId: string) {
    await this.prisma.updateMany('staff', { unique_id: staffId }, { admin_verify: '1' });
    return this.ok(null, 'Staff verified successfully');
  }

  async deleteStaff(staffId: string) {
    await this.prisma.deleteMany('staff', { unique_id: staffId });
    return this.ok(null, 'Staff deleted successfully');
  }

  async getSessions() {
    const [sessions, current] = await Promise.all([
      this.prisma.findMany<any>('session', { orderBy: { session: 'desc' } }),
      this.prisma.findFirst<any>('set_session_tbl'),
    ]);
  ...
  async setCurrentTerm(term: string) {
    await this.prisma.updateMany('set_term_tbl', {}, { set_term: term });
    return this.ok(null, 'Current term updated successfully');
  }

  async getPendingPayments() {
    const payments = await this.prisma.findMany<any>('scratch_card', {
  ...
  async verifyPayment(id: number) {
    const payment = await this.prisma.findFirst('scratch_card', { where: { scratch_card_id: id }, select: { scratch_card_id: true } });
    if (!payment) throw new NotFoundException('Payment record not found.');
    await this.prisma.updateMany('scratch_card', { scratch_card_id: id }, { verified: 'yes', admin_date: String(new Date()) });
    return this.ok(null, 'Payment verified successfully');
  }

  async getLibrary() {
    const library = await this.prisma.findMany<any>('library', { orderBy: [{ verify: 'asc' }, { date: 'desc' }] });
    return this.ok(await this.withStaffNames(library));
  }

  async approveLibrary(id: number) {
    await this.prisma.updateMany('library', { library_id: id }, { verify: '1' });
  ...
  async getClasses() {
    const classes = await this.prisma.findMany<any>('class', { orderBy: { class_id: 'asc' } });
    const staff = await this.prisma.findMany<any>('staff', { select: { unique_id: true, firstname: true, lastname: true } });
    ...
    const data = await Promise.all(classes.map(async cls => {
      ...
      return { ...cls, teacher_name: teacher ? `${teacher.firstname} ${teacher.lastname}` : null, student_count: await this.prisma.count('users', { class: cls.class }) };
    }));
    return this.ok(data);
  }

  async createClass(data: any) {
    await this.prisma.create('class', { class: data.class, class_teacher: data.class_teacher || '' });
    return this.ok(null, 'Class created successfully');
  }

  async updateClass(oldName: string, data: any) {
    await this.prisma.updateMany('class', { class: oldName }, { class: data.class, class_teacher: data.class_teacher || '' });
    return this.ok(null, 'Class updated successfully');
  }

  async deleteClass(name: string) {
    await this.prisma.deleteMany('class', { class: name });
  ...
  async createCourse(data: any) {
    await this.prisma.create('course', { courses: data.course, teacher: data.teacher || '' });
    return this.ok(null, 'Course created successfully');
  }

  async updateCourse(oldName: string, data: any) {
    await this.prisma.updateMany('course', { courses: oldName }, { courses: data.course, teacher: data.teacher || '' });
    return this.ok(null, 'Course updated successfully');
  }

  async deleteCourse(name: string) {
    await this.prisma.deleteMany('course', { courses: name });
  ...
  async getResults(q: any) {
    ...
    const [resultRows, users, classes, sessions] = await Promise.all([
      this.prisma.findMany<any>('result', { where }),
      this.prisma.findMany<any>('users', { where: usersWhere, select: { student_id: true, firstname: true, lastname: true, class: true, image: true } }),
      this.prisma.findMany('class', { orderBy: { class_id: 'asc' } }),
      this.prisma.findMany('session', { orderBy: { session: 'desc' }, select: { session: true } }),
    ]);
  ...
  async getStudentResults(studentId: string, q: any) {
    ...
    const [results, attendance] = await Promise.all([
      ...
      this.prisma.findFirst('attendance', { where: { student_id: studentId, session, term } }),
    ]);
  ...
  async approveResults(studentId: string, body: any) {
    ...
    await this.prisma.updateMany('result', { student_id: studentId, session, term }, { approved: '1' });
    await this.prisma.create('notifications', { user_id: studentId, user_type: 'student', title: 'Results Approved', message: `Your result for ${term} term, ${session} session has been approved.`, is_read: false, created_at: new Date() });
  ...
  async unapproveResults(studentId: string, body: any) {
    const session = body.session || await this.getCurrentSession();
    const term = body.term || await this.getCurrentTerm();
    await this.prisma.updateMany('result', { student_id: studentId, session, term }, { approved: '0' });
    await this.prisma.create('notifications', { user_id: studentId, user_type: 'student', title: 'Results Unapproved', message: `Your result for ${term} term, ${session} session has been unapproved.`, is_read: false, created_at: new Date() });
  ...
  async updatePrincipalComment(studentId: string, body: any) {
    ...
    if (existing) {
      await this.prisma.updateMany('attendance', { student_id: studentId, session, term }, { principal_comment: comment });
    } else {
      await this.prisma.create('attendance', { student_id: studentId, session, term, principal_comment: comment, present: '0', absent: '0' });
    }
    return this.ok({ student_id: studentId, session, term, principal_comment: comment }, 'Comment updated successfully');
  }

  async getSchoolDays() {
    return this.ok(await this.prisma.findMany('school_days', { orderBy: [{ session: 'desc' }, { term: 'asc' }] }));
  }
  ...
  async setSchoolDays(body: any) {
    ...
    const existing = await this.prisma.findFirst('school_days', { where: { session, term }, select: { id: true } });
    if (existing) await this.prisma.updateMany('school_days', { session, term }, { total_days: Number(total_days) });
    else await this.prisma.create('school_days', { session, term, total_days: Number(total_days) });
    return this.ok(null, 'School days set successfully');
  }

  async deleteSchoolDays(session: string, term: string) {
    await this.prisma.deleteMany('school_days', { session, term });
  ...
  private async generateStudentId(): Promise<string> {
    ...
    const last = await this.prisma.findFirst<any>('users', { orderBy: { user_id: 'desc' }, select: { student_id: true } });
  ...
  private async generateStaffId(): Promise<string> {
    const year = new Date().getFullYear().toString().slice(-2);
    const last = await this.prisma.findFirst<any>('staff', { orderBy: { staff_id: 'desc' }, select: { unique_id: true } });
  ...
  private async withStudentInfo<T extends { student_id?: string }>(items: T[]) {
    ...
    const users = await this.prisma.findMany<any>('users', { where: { student_id: { in: ids } }, select: { student_id: true, firstname: true, lastname: true, class: true } });
  ...
  private async withStaffNames<T extends { staff_id?: string }>(items: T[]) {
    ...
    const staff = await this.prisma.findMany<any>('staff', { where: { unique_id: { in: ids } }, select: { unique_id: true, firstname: true, lastname: true } });
    const byId = new Map(staff.map(s => [s.unique_id, s]));
