...
export class StudentService {
  ...
  async dashboard(user: any) {
    const [session, term, unread, assignments] = await Promise.all([
      ...
      this.getCurrentTerm(),
      this.db.count('notifications', "unique_id = ? AND status = 'unread'", [user.student_id]),
      this.db.query('SELECT * FROM assignments WHERE class = ? ORDER BY id DESC LIMIT 5', [user.class]),
    ]);
  ...
  async getResults(user: any, q: any) {
    ...
    const approved = await this.db.queryOne<any>('SELECT approved FROM result WHERE student_id = ? AND session = ? AND term = ? LIMIT 1', [user.student_id, session, term]);
    if (!approved?.approved) {
      return this.ok({ results: [], approved: false, message: 'Results not yet approved by admin', student: { student_id: user.student_id, firstname: user.firstname, lastname: user.lastname, class: user.class } });
    }

    const results = await this.db.query('SELECT *, (test_score + exam_score) as total_score FROM result WHERE student_id = ? AND session = ? AND term = ?', [user.student_id, session, term]);
    const attendance = await this.db.queryOne('SELECT * FROM attendance WHERE student_id = ? AND session = ? AND term = ?', [user.student_id, session, term]);
    const classSize = await this.db.count('result', 'session = ? AND term = ? AND approved = 1 AND student_id IN (SELECT student_id FROM users WHERE class = ?)', [session, term, user.class]);
    const teacher = await this.db.queryOne<any>('SELECT firstname, lastname, image FROM staff WHERE class = ? LIMIT 1', [user.class]);
    const principal = await this.db.queryOne<any>("SELECT firstname, lastname, image FROM staff WHERE course = 'principal' OR class = 'principal' LIMIT 1");

    return this.ok({ results, attendance, class_size: classSize, approved: true, session, term, teacher: teacher ? { name: `${teacher.firstname} ${teacher.lastname}`, image: teacher.image } : null, principal: principal ? { name: `${principal.firstname} ${principal.lastname}`, image: principal.image } : null, student: { student_id: user.student_id, firstname: user.firstname, lastname: user.lastname, class: user.class, image: user.image } });
  }

  async getAssignments(user: any) {
    return this.ok(await this.db.query('SELECT * FROM assignments WHERE class = ? ORDER BY id DESC', [user.class]));
  }

  async getLibrary(user: any) {
    return this.ok(await this.db.query("SELECT * FROM library WHERE class = ? AND verify = '1' ORDER BY id DESC", [user.class]));
  }
  ...
  async getNotifications(user: any) {
    return this.ok(await this.db.query('SELECT * FROM notifications WHERE unique_id = ? ORDER BY id DESC', [user.student_id]));
  }

  async markNotificationsRead(user: any) {
    await this.db.update('notifications', { status: 'read' }, "unique_id = ? AND status = 'unread'", [user.student_id]);
